...
# ==========================================================
# STAGE 1: 오프닝 서술 (외형에 따른 묘사 다변화 반영)
# ==========================================================
    ...
    # --- 머리 길이에 따른 가변 서술 조건문 ---
    h_color = st.session_state.hair_color
    h_length = st.session_state.hair_length
    
    if h_length in ["장발", "세미롱", "중단발"]:
        hair_desc = f"어디선가 갑작스레 불어닥친 밤바람에 당신의 {h_color} {h_length} 머리가 세차게 휘날리며 어두운 시야를 가립니다."
    else:
        hair_desc = f"어디선가 갑작스레 불어닥친 밤바람에 당신의 짧은 {h_color} 머리칼이 거칠게 흩날립니다."
...
# ==========================================================
    ...
    # --- 눈 색에 따른 가변 서술 적용 ---
    e_color = st.session_state.eye_color
    st.write(f"숨이 턱 막히는 공포 속에서 흐릿한 시야를 간신히 넓혀봅니다. 두려움 혹은 경계심으로 잘게 떨리는 당신의 {e_color} 눈동자가 허공의 백색 형체를 똑바로 담아냅니다. 눈앞까지 다가온 것은 다름 아닌 허공을 일렁이는 거대한 하얀 천이었습니다.")
