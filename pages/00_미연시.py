...
st.markdown("""
    ...
    div[data-testid="stNotification"] {
        ...
        border: 1px solid #FF69B4 !important; /* 분홍색 테두리 */
        border-radius: 5px;
    }
...
# ==========================================================
# STAGE 5: 세 번째 상황 (지속 확장을 위해 열어둔 스테이지)
# ==========================================================
elif st.session_state.stage == 5:
    st.subheader("그 이후의 상황")
    
    st.write("(이전 선택지에 따른 결과와 다음 상황 묘사를 이곳에 적어주세요.)")
    
    st.info("기시 유스케: (대사)")
    
    # 엔딩 대신 다음 단계로 계속 이어갈 수 있도록 구조를 유지합니다.
    if st.button("다음 상황으로"):
        # 필요시 st.session_state.stage = 6 등으로 넘어가도록 수정 가능합니다.
