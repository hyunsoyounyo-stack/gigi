...
# ==========================================================
# STAGE 1: 오프닝 서술 (외형별 형용사 묘사 다변화 반영)
# ==========================================================
    ...
    # --- 선택지 한글 수식어 변환 딕셔너리 ---
    hair_color_map = {
        "흑발": "칠흑 같은", "백발": "눈부시게 하얀", "금발": "금빛의", 
        "갈발": "부드러운 갈색의", "은발": "달빛을 닮은 은색의", 
        "적발": "타오르는 듯한 붉은색의", "청발": "깊은 바다 같은 청색의"
    }
    
    h_adj = hair_color_map.get(st.session_state.hair_color, "그윽한 색의")
...
# ==========================================================
    ...
    # --- 눈 색 한글 수식어 변환 딕셔너리 ---
    eye_color_map = {
        "흑안": "어둠이 내려앉은 검은", "적안": "붉게 번뜩이는", "벽안": "시린 푸른빛의", 
        "자안": "신비로운 자색의", "녹안": "싱그러운 녹색의", "금안": "은은하게 빛나는 금색의", 
        "역안": "기이하게 뒤틀린"
    }
    e_adj = eye_color_map.get(st.session_state.eye_color, "흔들리는")
    
    st.write(f"숨이 턱 막히는 공포 속에서 흐릿한 시야를 간신히 넓혀봅니다. 두려움 혹은 경계심으로 잘게 떨리는 당신의 {e_adj} 눈동자가 허공의 백색 형체를 똑바로 담아냅니다. 눈앞까지 다가온 것은 다름 아닌 허공을 일렁이는 거대한 하얀 천이었습니다.")
